...
- Result: **ULTRAQA COMPLETE: goal met after 2 cycles.** Cycle 1 found and fixed one product defect. A separate harness-lifetime setup failure was corrected and was not classified as a product defect.

## Scenario matrix

| ID | User/attacker model | Scenario | Command/harness | Expected signal | Actual result | Status | Evidence | Cleanup |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| ADV-E2E-001 | Normal seller | One-command mock startup, six stages, SSE, export, cancel/rerun | `./overseaark test` | All native suites pass | 46 backend, 8 frontend, 14 HTTP E2E; build and lifecycle passed | Pass | Terminal output and CI-equivalent commands below | Backend stopped by lifecycle test |
...
| ADV-E2E-011 | Real DGX operator | Four complete real-model campaigns; verify 480p video, audio QC, export integrity and process release | DGX API, `ffprobe`, zip integrity, SHA-256 capture | Real outputs are valid and timings are reported honestly | Four campaigns completed; run 4 was 590.003s with similarity 0.933/1.0/1.0, 854x480 H.264/AAC, valid 23-member zip, no degraded output | Pass; one sub-10 run | Evidence directory and DGX campaign JSON | LLM stopped before heavy adapters; only FastAPI remained |

## Commands run

- `[0] ./overseaark test` — full baseline: backend 46, frontend 8, TypeScript/Vite build, one-click lifecycle, HTTP E2E 14.
...
- The <=10-minute full-flow target was met once by run 4 after the 6-step Step1X change. Two additional consecutive qualifying campaigns are still required before AT-001 can be marked complete.
