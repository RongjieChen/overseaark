...
load_env() {
  if [[ -f "$REPO_DIR/.env" ]]; then
    ...
    local override_names=(
      ...
      OVERSEAARK_LLAMA_REVISION OVERSEAARK_LLAMA_CPP_DIR
      OVERSEAARK_LLAMA_BUILD_DIR OVERSEAARK_LLAMA_SERVER
      OVERSEAARK_LLAMA_MODEL_DIR OVERSEAARK_LLAMA_MODEL
      OVERSEAARK_LLAMA_MMPROJ OVERSEAARK_LLAMA_SERVED_MODEL
      OVERSEAARK_LLAMA_PORT OVERSEAARK_LLAMA_STARTUP_TIMEOUT
      OVERSEAARK_LLAMA_CONTEXT_SIZE OVERSEAARK_LLAMA_PARALLEL
  ...
  export OVERSEAARK_LLAMA_REVISION="${OVERSEAARK_LLAMA_REVISION:-76f46ad29d61fd8c1401e8221842934bf62a6064}"
  export OVERSEAARK_LLAMA_CPP_DIR="${OVERSEAARK_LLAMA_CPP_DIR:-$REPO_DIR/vendor/llama.cpp}"
  export OVERSEAARK_LLAMA_BUILD_DIR="${OVERSEAARK_LLAMA_BUILD_DIR:-$OVERSEAARK_LLAMA_CPP_DIR/build}"
  export OVERSEAARK_LLAMA_SERVER="${OVERSEAARK_LLAMA_SERVER:-$OVERSEAARK_LLAMA_BUILD_DIR/bin/llama-server}"
  export OVERSEAARK_LLAMA_MODEL_DIR="${OVERSEAARK_LLAMA_MODEL_DIR:-$OVERSEAARK_MODELS_DIR/qwen/qwen3.6-35b-a3b-gguf}"
  export OVERSEAARK_LLAMA_MODEL="${OVERSEAARK_LLAMA_MODEL:-$OVERSEAARK_LLAMA_MODEL_DIR/Qwen3.6-35B-A3B-Q4_K_M.gguf}"
  export OVERSEAARK_LLAMA_MMPROJ="${OVERSEAARK_LLAMA_MMPROJ:-$OVERSEAARK_LLAMA_MODEL_DIR/mmproj-Qwen3.6-35B-A3B-BF16.gguf}"
  export OVERSEAARK_LLAMA_SERVED_MODEL="${OVERSEAARK_LLAMA_SERVED_MODEL:-ggml-org/Qwen3.6-35B-A3B-GGUF}"
  export OVERSEAARK_LLAMA_PORT="${OVERSEAARK_LLAMA_PORT:-8011}"
  export OVERSEAARK_LLAMA_STARTUP_TIMEOUT="${OVERSEAARK_LLAMA_STARTUP_TIMEOUT:-900}"
  export OVERSEAARK_LLAMA_CONTEXT_SIZE="${OVERSEAARK_LLAMA_CONTEXT_SIZE:-32768}"
  export OVERSEAARK_LLAMA_PARALLEL="${OVERSEAARK_LLAMA_PARALLEL:-1}"
  export OVERSEAARK_LLAMA_API_KEY_FILE="${OVERSEAARK_LLAMA_API_KEY_FILE:-$OVERSEAARK_PID_DIR/llama-api-key}"
  export OVERSEAARK_LLM_BASE_URL="${OVERSEAARK_LLM_BASE_URL:-http://127.0.0.1:$OVERSEAARK_LLAMA_PORT}"
...
local_runtime_env() {
  ...
  if [[ "$OVERSEAARK_ADAPTER_MODE" == "command" ]]; then
    ...
    export OVERSEAARK_LLM_CONTROL_COMMAND="${OVERSEAARK_LLM_CONTROL_COMMAND:-$REPO_DIR/overseaark llm}"
    ...
    export OVERSEAARK_ASR_COMMAND="${OVERSEAARK_ASR_COMMAND:-$REPO_DIR/.venv-asr/bin/python $SCRIPT_DIR/adapters/asr_nemo.py}"
    export OVERSEAARK_TTS_COMMAND="${OVERSEAARK_TTS_COMMAND:-$REPO_DIR/.venv-tts/bin/python $SCRIPT_DIR/adapters/tts_magpie.py}"
  fi
}
...
validate_offline_runtime() {
  ...
  [[ "$OVERSEAARK_LLM_BASE_URL" == "http://127.0.0.1:$OVERSEAARK_LLAMA_PORT" || \
     "$OVERSEAARK_LLM_BASE_URL" == "http://localhost:$OVERSEAARK_LLAMA_PORT" ]] || \
