...
native vLLM 迁移已获得一轮完整低于十分钟的实机证据：Run9 用时 580.147 秒，六阶段全部一次成功。它证明 native vLLM 与 Step1X、Cosmos、Magpie、Nemotron 的串行切换可以在目标时间内完成，但 PRD 中“连续三轮”的更严格标准仍需两轮后续达标证据，不对单轮结果过度声称。
