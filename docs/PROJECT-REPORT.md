...
项目已在指定 DGX Spark 上完成四次真实模型全流程。第一轮在中间修复后通过阶段重跑完成；第二、三轮无中断完成，分别用时 10 分 34 秒和 10 分 45 秒。将 Step1X 默认值由 8 步调整为经独立基准验证的 6 步后，第四轮六个阶段全部一次成功，端到端用时 590.003 秒（约 9 分 50 秒）。该轮生成 15 秒、854×480 的 H.264/AAC 视频，23 文件 ZIP 通过完整性检查，中文、英文、日文回听相似度分别为 0.933、1.0、1.0，Step1X 和 Cosmos 均为真实模型输出，没有使用降级视频。

除正常测试外，我们按 UltraQA 方式构造了恶意和故障场景：伪造 PNG、空文件和超大文件、路径穿越式字段、Unicode 提示注入、连续取消与重跑、两个 Campaign 同时竞争模型、SSE 断点续传、ASR 永久低相似度、Cosmos 降级伪装、adapter 超时后的子进程残留，以及输出写着 SUCCESS 但进程返回非零。审查发现并修复了一个真实问题：后端崩溃可能让 SQLite 中的运行任务永久卡住。修复后，21 个核心对抗场景连续三轮全部通过；完整本地套件包括 46 个后端测试、8 个前端测试和 14 个 HTTP Mock E2E。
...
当前性能目标已有一轮实测达标。第二、三轮均成功，但分别超出“十分钟内完成”目标 34 秒和 45 秒；Step1X 的 6 步独立 DGX 基准为 176.3 秒，第四轮完整流程随后实测为 590.003 秒，证明优化在端到端场景有效。目前只能宣称“一次实测低于十分钟”；PRD 中“连续三轮”的更严格验收标准仍需再补两轮达标证据。后续还可以通过让 TTS/ASR 批量处理三种语言、减少模型重复加载来进一步压缩冷启动开销。
